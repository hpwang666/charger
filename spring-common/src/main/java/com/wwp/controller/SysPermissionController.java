...
import com.wwp.common.util.PermissionDataUtil;
...
import com.wwp.model.SysPermissionTree;
...
public class SysPermissionController {
	...
	public Result<List<SysPermissionTree>> list() {
		...
		try {
			...
			List<SysPermission> list = sysPermissionService.queryAllPermissions();
	...
	public Result<List<SysPermissionTree>> getSystemMenuList() {
		...
		try {
			...
			List<SysPermission> list = sysPermissionService.querySystemMenuList();
	...
	public Result<List<SysPermissionTree>> getSystemSubmenu(@RequestParam("parentId") String parentId){
		...
		try{
			...
			List<SysPermission> list = sysPermissionService.querySystemSubmenu(parentId);
			List<SysPermissionTree> sysPermissionTreeList = new ArrayList<SysPermissionTree>();
			for(SysPermission sysPermission : list){
				SysPermissionTree sysPermissionTree = new SysPermissionTree(sysPermission);
				sysPermissionTreeList.add(sysPermissionTree);
	...
	public Result<?> getUserPermissionByToken(HttpServletRequest request)throws Exception {
		...
		if(sysRoles.size() == 0) {
			permissionList = null;//queryPermissionByIdList(null);
		} else {
			String roleId = sysRoles.get(0).getId();//TODO 只获取第一个角色？
			//查询这个角色对应的权限

			List<SysRolePermission> rolePermissionList = sysRolePermissionService.queryRolePermissionByRoleId(roleId);
			List<String> perIdList = rolePermissionList.stream()
					.map(SysRolePermission::getPermissionId)
					.collect(Collectors.toList());
			if(perIdList.isEmpty()) {
				permissionList = new ArrayList<>();
			} else {
				permissionList = queryPermissionByIdList(perIdList);
			}
		}
		try {
			//添加首页路由
//			PermissionDataUtil.addIndexPage(permissionList);
			JSONObject json = new JSONObject();
			JSONArray menujsonArray = new JSONArray();
			this.getPermissionJsonArray(menujsonArray, permissionList, null);
			JSONArray buttonjsonArray = new JSONArray();
			this.getButtonJsonArray(buttonjsonArray, permissionList);
			//查询所有的权限


			//搞成auth?
			//query.eq(SysPermission::getStatus, "1");
			List<SysPermission> allAuthList = sysPermissionService.querySysAuthButton();
			JSONArray authjsonArray = new JSONArray();
			this.getAuthJsonArray(authjsonArray, allAuthList);
			//路由菜单
			json.put("menu", menujsonArray);
			//按钮权限
			json.put("button", buttonjsonArray);
			//访问权限
	...
	private List<SysPermission> queryPermissionByIdList(List<String> idList) {
		return sysPermissionService.queryPermissionByIdList(idList);
	}
	...
	public Result<List<String>> queryRolePermission(@RequestParam(name = "roleId", required = true) String roleId) {
		...
		try {
			List<SysRolePermission> list = sysRolePermissionService.queryRolePermissionByRoleId(roleId);
	 ...
	 */
	private void getAuthJsonArray(JSONArray jsonArray, List<SysPermission> allList) {
		JSONObject json = null;
		for (SysPermission permission : allList) {
			if(permission.getResourceType().equals("auth") ){
				json = new JSONObject();
				json.put("action", permission.getPermission());
	 ...
	 * @param metaList
	 */
	private void getButtonJsonArray(JSONArray jsonArray, List<SysPermission> metaList) {
		for (SysPermission permission : metaList) {
			if (oConvertUtils.isEmpty(permission.getResourceType())) {
				continue;
			}
			JSONObject json = null;
			if(permission.getResourceType().equals("button") ) {//&&CommonConstant.STATUS_1.equals(permission.getStatus())
				json = new JSONObject();
				json.put("action", permission.getPermission());
				//json.put("type", permission.getPermsType());
				json.put("describe", permission.getName());
	...
	private void getPermissionJsonArray(JSONArray jsonArray, List<SysPermission> metaList, JSONObject parentJson) {
		for (SysPermission permission : metaList) {
			...
			if (parentJson == null && oConvertUtils.isEmpty(tempPid)) {
				...
				if (permission.getResourceType().equals("button")) {
					...
					// 类型( 0：一级菜单 1：子菜单 2：按钮 )
				} else if (permission.getResourceType().equals("menu1") || permission.getResourceType().equals("menu0")) {
	 ...
	 */
		...
		if (permission.getResourceType().equals("button")) {
			...
			if (oConvertUtils.isNotEmpty(permission.getComponent())) {
				json.put("name", permission.getComponent());
			} else {
